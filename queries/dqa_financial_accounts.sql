WITH
  ...
  SELECT 
    ...
    COUNTIF(account_source = "PLAID" AND account_id IS NULL) AS null_plaid_accounts,
    ...
    COUNTIF(account_source = "PLAID" AND institution_id IS NULL) AS null_institution_id,
    COUNTIF(account_source = "PLAID" AND institution_name IS NULL) AS null_institution_name,
    COUNTIF(balance.current IS NULL) AS null_balance,
    COUNTIF(balance.currency_code IS NULL) AS null_currency_code,
    COUNTIF(account_source = "PLAID" AND update_type IS NULL) AS null_plaid_update_type,
    COUNTIF(account_source = "PLAID" AND ARRAY_LENGTH(products) = 0) AS null_plaid_products,
    COUNTIF(account_source = "PLAID" AND ARRAY_LENGTH(billed_products) = 0) AS null_plaid_billed_products,
