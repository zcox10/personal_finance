...
CREATE TEMP FUNCTION format_size_bytes (size_bytes INT64) RETURNS STRING AS (
  CASE
...
FROM
  (
    ...
    FROM
      `zsc-personal.personal_finance.__TABLES__`
    UNION ALL
    SELECT
      dataset_id,
      table_id,
      ...
      format_size_bytes (size_bytes) AS table_size,
    FROM
      `zsc-personal.budget_values.__TABLES__`
  )
...
QUALIFY
  ROW_NUMBER() OVER (
    PARTITION BY
      REGEXP_EXTRACT(table_id, IF(dataset_id IN ("personal_finance"), r'(.*)\d{8}$', r'(.*)\d{6}$'))
    ORDER BY
      table_id DESC
  ) = 1
ORDER BY
