WITH
  ...
  transaction_categories AS (
    ...
    FROM
      plaid_transactions
    WHERE
      detailed NOT IN ("TRANSFER_OUT_INVESTMENT_AND_RETIREMENT_FUNDS")
  ),
  ...
  missing_categories_data AS (
    SELECT
      STRING_AGG(
        CONCAT(category_raw, " - ", subcategory_raw),
        "; "
        ORDER BY
          CONCAT(category_raw, " - ", subcategory_raw)
    ...
    FROM
      transaction_categories t
      LEFT JOIN budget_values_categories b USING (category_raw, subcategory_raw)
    WHERE
