WITH
  partition_0d AS (
    SELECT
      ...
      COUNTIF(
        item_id IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG")
      ) AS null_item_ids,
      COUNTIF(
        account_name IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG")
      ) AS null_account_names,
      COUNTIF(
        account_subname IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG")
      ) AS null_account_subnames,
      COUNTIF(
        account_type IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG")
      ) AS null_account_types,
      COUNTIF(
        budget_amount IS NULL
        AND metric_category IN ("TRANSACTIONS_AGG")
      ) AS null_budget_amounts,
      COUNTIF(
        transactions_count IS NULL
        AND metric_category IN ("TRANSACTIONS_AGG")
      ) AS null_transaction_counts,
      COUNTIF(
        currency_code IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG")
      ) AS null_currency_codes,
      -- null: tranasactions
      COUNTIF(
        transaction_type IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_transaction_types,
      COUNTIF(
        category_raw IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_category_raws,
      COUNTIF(
        subcategory_raw IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_subcategory_raws,
      COUNTIF(
        category_updated IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_category_updated,
      COUNTIF(
        subcategory_updated IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_subcategory_updated,
      COUNTIF(
        payment_channel IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_payment_channels,
      COUNTIF(
        merchant_name IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_merchant_names,
      COUNTIF(
        name_raw IS NULL
        AND metric_category IN ("TRANSACTIONS")
      ) AS null_name_raw,
      ...
      COUNTIF(
        category IS NULL
        AND metric_category IN ("TRANSACTIONS", "TRANSACTIONS_AGG")
      ) AS null_categories,
      COUNTIF(
        subcategory IS NULL
        AND category NOT IN ("Balance")
        AND metric_category IN ("TRANSACTIONS", "TRANSACTIONS_AGG")
      ) AS null_subcategories,
      COUNTIF(
        final_category IS NULL
        AND metric_category IN ("TRANSACTIONS", "TRANSACTIONS_AGG")
      ) AS null_final_categories,
      COUNTIF(
        full_category IS NULL
        AND metric_category IN ("TRANSACTIONS", "TRANSACTIONS_AGG")
      ) AS null_full_categories,
      -- null: transactions_agg and accounts
      COUNTIF(
        account_id IS NULL
        AND metric_category NOT IN ("TRANSACTIONS_AGG", "ACCOUNTS")
      ) AS null_account_ids,
      -- null: investments
      COUNTIF(
        institution_price IS NULL
        AND metric_category IN ("INVESTMENTS")
      ) AS null_institution_prices,
      COUNTIF(
        quantity IS NULL
        AND metric_category IN ("INVESTMENTS")
      ) AS null_quantities,
      COUNTIF(
        ...
        AND metric_category IN ("INVESTMENTS")
      ) AS null_security_types,
      COUNTIF(
        security_name IS NULL
        AND metric_category IN ("INVESTMENTS")
      ) AS null_security_names,
      -- null: transactions and investments
      COUNTIF(
        transaction_id IS NULL
        AND metric_category IN ("TRANSACTIONS", "INVESTMENTS")
      ...
      (
        ...
        FROM
          (
            SELECT
              CONCAT(transaction_date, " - ", transaction_id)
            FROM
              `{full_table_name_0d}`
            WHERE
          ...
          (
            SELECT
              CONCAT(
                ...
                " - ",
                item_id,
                IF(account_id IS NULL, "", CONCAT(" - ", account_id))
              )
            FROM
              `{full_table_name_0d}`
            WHERE
      ...
      SUM(
        IF(
          metric_category = "ACCOUNTS"
          AND partition_date = PARSE_DATE("%Y%m%d", "{table_suffix_0d}"),
          actual_amount,
          0
        )
      ) AS total_account_value_0d, -- compare financial_accounts
    FROM
      `{full_table_name_0d}`
  ),
  partition_1d AS (
  ...
  financial_accounts AS (
    SELECT
      SUM(IF(account_type = "credit", balance.current * -1, balance.current)) AS total_account_value_financial_accounts
    FROM
...
SELECT
  ...
  ROUND(
    (
      (total_account_summed_value_0d - total_account_summed_value_1d) / ABS(total_account_summed_value_1d)
    ) * 100,
    2
  ) AS total_account_summed_value_pct_chg,
  ROUND(
    ABS(
      (
        (total_account_summed_value_0d - total_account_summed_value_1d) / ABS(total_account_summed_value_1d)
    ...
    2
  ) AS total_account_summed_value_pct_chg_abs,
