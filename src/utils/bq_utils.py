...
class BqUtils:
    ...
    def get_date_offset(self, date_str, partition_format):
        """
        Calculate the offset from the current date to the given date based on the partition format.

        Args:
            date_str (str): The target date as a string.
            partition_format (str): The format of the partition ('YYYYMMDD', 'YYYYMM', 'YYYYMMDDHH', or 'YYYYMMDDTHH').
        ...
        if partition_format == "YYYYMMDD":
            ...
            target_date = dt.strptime(date_str, "%Y-%m")
            delta = relativedelta(target_date, current_date)
            return delta.years * 12 + delta.months

        elif partition_format == "YYYYMMDDHH":
            target_date = dt.strptime(date_str, "%Y-%m-%dT%H")
            delta = target_date - current_date
