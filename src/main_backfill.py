...
if backfill:
    ...
    plaid_client.create_accounts_bq_table(
        access_tokens=PLAID_ACCESS_TOKENS, plaid_country_codes=PLAID_COUNTRY_CODES, confirm=confirm
    )

    # Create a new plaid_cursors_YYYYMMDD table with access_token, item_id, and next_cursor
    plaid_client.create_cursors_bq_table(offset_days=offset_days, confirm=confirm)
...
plaid_client.create_temp_cursors_bq_table(confirm=confirm)

# create empty plaid_transactions_YYYYMMDD and plaid_removed_transactions_YYYYMMDD to upload transactions to
plaid_client.create_empty_transactions_bq_table(offset_days=offset_days, confirm=confirm)
plaid_client.create_empty_removed_bq_table(offset_days=offset_days, confirm=confirm)
...
# ### TEST load_df_to_bq()
# data = {"name": ["Alice", "Bob", "Charlie"], "age": [25, 30, 35], "city": ["New York", "San Francisco", "Seattle"]}

# test_df = pd.DataFrame(data)

# table_name = "zsc-personal.personal_finance.test_load_df_to_bq_20240414"
# test_job = bq.load_df_to_bq(df=test_df, full_table_name=table_name, write_disposition="WRITE_TRUNCATE")
...
#     # print(table_id)
#     bq.delete_all_partitions(project_id, dataset_id, table_id, confirm=confirm)
